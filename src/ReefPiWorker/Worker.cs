...
namespace ReefPiWorker
{
    public class Worker : BackgroundService
    {
        ...
        public Worker(
            ...
            (_logger, _arduinoUnoR3FirmataCommandsWrapper, _reefFactoryScrapper, _influxDbClient) = 
            (logger, arduinoUnoR3FirmataCommandsWrapper, reefFactoryScrapper, influxDbClient);
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            while (!stoppingToken.IsCancellationRequested)
            {
                try
                {
                    ...
                    if (data == null)
                    {
                        _logger.LogInformation(
                            $"{DateTime.Now.ToShortTimeString()} Kh measurement in progress, skipping...");
                    }
                    else
                    {
                        _logger.LogInformation(
                            $"At {DateTime.Now.ToShortTimeString()} temperature is {temp}, humidity is {hum}" +
                            Environment.NewLine +
                ...
                {
                    _logger.LogError("Error in worker", e);
                }
            }
        }
    }    
